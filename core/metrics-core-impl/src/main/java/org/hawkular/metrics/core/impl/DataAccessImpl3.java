...
public class DataAccessImpl3 {
    ...
    public DataAccessImpl3(Session session) {
    ...
    public ListenableFuture<List<ResultSet>> findData(Metric<Double> metric, long startTime, long endTime, Order
        ...
        if (order == Order.ASC) {
            for (long dpart : dpartRangeASC(startTime, endTime)) {
                futures.add(session.executeAsync(findGaugeDataByDateRangeExclusiveASC.bind(metric.getTenantId(),
            ...
            for (long dpart : dpartRangeDESC(startTime, endTime)) {
                futures.add(session.executeAsync(findGaugeDataByDateRangeExclusive.bind(metric.getTenantId(),
                        MetricType.GAUGE.getCode(), metric.getId().getName(), metric.getId().getInterval().toString(),
                        dpart, getTimeUUID(startTime), getTimeUUID(endTime))));
            }
        }
    ...
    public ListenableFuture<List<ResultSet>> findData(String tenantId, MetricId id, MetricType type, long startTime,
        ...
        if (includeWriteTime) {
            for (long dpart : dpartRangeDESC(startTime, endTime)) {
                futures.add(session.executeAsync(findGaugeDataWithWriteTimeByDateRangeExclusive.bind(tenantId,
                        type.getCode(),
                        id.getName(),
            ...
            for (long dpart : dpartRangeDESC(startTime, endTime)) {
                futures.add(session.executeAsync(findGaugeDataByDateRangeExclusive.bind(tenantId, type.getCode(),
                        id.getName(),
                        id.getInterval().toString(), dpart, getTimeUUID(startTime), getTimeUUID(endTime))));
            }
        }
    ...
    public ListenableFuture<List<ResultSet>> findAvailabilityData(Metric<AvailabilityType> metric, long startTime,
        ...
        if (includeWriteTime) {
            for (long dpart : dpartRangeDESC(startTime, endTime)) {
                futures.add(session.executeAsync(findAvailabilitiesWithWriteTime.bind(metric.getTenantId(),
            ...
            for (long dpart : dpartRangeASC(startTime, endTime)) {
                futures.add(session.executeAsync(findAvailabilities.bind(metric.getTenantId(),
