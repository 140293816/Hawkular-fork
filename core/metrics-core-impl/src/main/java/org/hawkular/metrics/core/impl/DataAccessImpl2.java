...
public class DataAccessImpl2 {
    ...
    public List<Observable<ResultSet>> findData(String tenantId, MetricId id, MetricType type, long startTime,
            long endTime) {
        return findData(tenantId, id, type, startTime, endTime, false);
    }

    public List<Observable<ResultSet>> findCounterData(String tenantId, MetricId id, long startTime, long endTime) {
        List<Observable<ResultSet>> result = new ArrayList<Observable<ResultSet>>();
        for (long dpart : dpartRangeDESC(startTime, endTime)) {
            result.add(rxSession.execute(findCounterDataExclusive.bind(tenantId, COUNTER.getCode(), id.getName(),
                    id.getInterval().toString(), dpart, UUIDs.startOf(startTime), UUIDs.endOf(endTime))));
        }
        return result;
    }

    public List<Observable<ResultSet>> findData(Metric<Double> metric, long startTime, long endTime, Order
            order) {
        List<Observable<ResultSet>> result = new ArrayList<Observable<ResultSet>>();
        if (order == Order.ASC) {
            for (long dpart : dpartRangeASC(startTime, endTime)) {
                result.add(rxSession.execute(findGaugeDataByDateRangeExclusiveASC.bind(metric.getTenantId(),
            ...
            for (long dpart : dpartRangeDESC(startTime, endTime)) {
                result.add(rxSession.execute(findGaugeDataByDateRangeExclusive.bind(metric.getTenantId(),
                        MetricType.GAUGE.getCode(), metric.getId().getName(), metric.getId().getInterval().toString(),
                        dpart, getTimeUUID(startTime), getTimeUUID(endTime))));
            }
        }
    ...
    public List<Observable<ResultSet>> findData(String tenantId, MetricId id, MetricType type, long startTime,
        ...
        if (includeWriteTime) {
            for (long dpart : dpartRangeDESC(startTime, endTime)) {
                result.add(rxSession.execute(findGaugeDataWithWriteTimeByDateRangeExclusive.bind(tenantId,
                        type.getCode(),
                        id.getName(),
            ...
            for (long dpart : dpartRangeDESC(startTime, endTime)) {
                result.add(rxSession.execute(findGaugeDataByDateRangeExclusive.bind(tenantId, type.getCode(),
                        id.getName(),
                        id.getInterval().toString(), dpart, getTimeUUID(startTime), getTimeUUID(endTime))));
            }
        }
    ...
    public List<Observable<ResultSet>> findAvailabilityData(Metric<AvailabilityType> metric, long startTime, long
            endTime) {
    ...
    public List<Observable<ResultSet>> findAvailabilityData(Metric<AvailabilityType> metric, long startTime, long
        ...
        if (includeWriteTime) {
            for (long dpart : dpartRangeDESC(startTime, endTime)) {
                result.add(rxSession.execute(findAvailabilitiesWithWriteTime.bind(metric.getTenantId(),
            ...
            for (long dpart : dpartRangeASC(startTime, endTime)) {
                result.add(rxSession.execute(findAvailabilities.bind(metric.getTenantId(),
                        MetricType.AVAILABILITY.getCode(),
                        metric.getId().getName(), metric.getId().getInterval().toString(), dpart,
                        getTimeUUID(startTime),
                        getTimeUUID(endTime))));
            }
        }
        return result;
    ...
    public List<Observable<ResultSet>> findAvailabilityData(String tenantId, MetricId id, long startTime, long endTime) {
        List<Observable<ResultSet>> result = new ArrayList<Observable<ResultSet>>();
        for (long dpart : dpartRangeASC(startTime, endTime)) {
            result.add(rxSession.execute(findAvailabilities.bind(tenantId, MetricType.AVAILABILITY.getCode(),
                    id.getName(), id
    ...
    private List<Long> dpartRangeDESC(long startTime, long endTime) {
    ...
    private List<Long> dpartRangeASC(long startTime, long endTime) {
        ArrayList<Long> range = new ArrayList<Long>();
